...
% ---------------------------Transmitting Phase----------------------------
signal_transmit_digital= [transmitter(1).yiq_field1; transmitter(1).yiq_field2];
for a = 2 : framesNum
    signal_transmit_digital(end+1: end+height*width, :) = [transmitter(a).yiq_field1; transmitter(a).yiq_field2];
end
% Generate analog signal
n = framesNum*width*height;
